...
def disp(img_l, img_r, scan_size, scale):
    # disp(left image, right image, window size, scan range factor)
...
def stereo(left, right, win_size, scan_size):
    img_left = cv2.imread(left)
    img_right = cv2.imread(right)
    img_left = np.transpose(img_left, (2, 0, 1))
    img_right = np.transpose(img_right, (2, 0, 1))
    img_dis = disp(img_left[0], img_right[0], win_size, scan_size)
    cv2.imshow('disparity map', img_dis)
    cv2.waitKey(0)
...
if __name__ == "__main__":
    ...
    start = time.time()
    stereo('pentagon_left.bmp', 'pentagon_right.bmp', 7, 3)
    end = time.time()
